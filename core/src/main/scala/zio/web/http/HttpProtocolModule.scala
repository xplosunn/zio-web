...
trait HttpProtocolModule extends ProtocolModule {
  ...
  override def makeClient[M >: MaxMetadata <: MinMetadata, A](
    ...
    ZLayer.service[ClientConfig].map {
      hasClientConfig =>

        val clientService: ClientService[A] = new ClientService[A] {
          override def invoke[M, Request, Response, H](endpoint: Endpoint[M, Request, Response, H], request: Request)(implicit ev: A <:< Endpoint[M, Request, Response, H]): Task[Response] = {
            ...
            val con = new URL(s"${hasClientConfig.get.host}:${hasClientConfig.get.host}/${endpointUrlPart.stripPrefix("/")}")
              .openConnection
              .asInstanceOf[HttpURLConnection]

            requestToJson(request).foreach {
              strEncodedPayload =>
            ...
            if (con.getResponseCode == HttpURLConnection.HTTP_OK) {
              ZManaged.fromAutoCloseable(ZIO(con.getInputStream)).use {
                inputStream =>
